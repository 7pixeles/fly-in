...
class Simulator:
    ...
    def exe(
        ...
        # Inicializar estado
        turn = 0
        delivered: set[int] = set()  # IDs de drones que llegaron
        ...
        if self.verbose:
            print(f"Simulación iniciada: {len(drones)} drones")
            print(
                f"Zona inicio: {start.name}, Zona fin: {network.end_zone.name}"
                )
        ...
        while len(delivered) < len(drones):
            ...
            for dron in drones:
                ...
                if self._can_move(network, dron, next_zone):
            ...
            if movements_this_turn:
                ...
                if len(delivered) < len(drones):
                    raise SimulationError(
                        f"Deadlock en turno {turn}: "
                        f"{len(drones) - len(delivered)} drones bloqueados")

        # Limpiar ocupancias (devolver drones a 0)
        self._clean_occupancy(network)

        # Calcular métricas
        self.metrics = self._calc_metrics(turn, drones)
    ...
    def _calc_metrics(self, final_turn: int, drones: list[Drone]
                      ) -> dict[str, int | float]:
        ...
        average_steps_dron = total_cost / len(drones) if drones else 0
