...
class SimulationState(BaseModel):
    ...
    Atributos:
        ...
        connection_occupancy: Mapa de conexiones a número de drones en tránsito.
        movements_this_turn: Lista de movimientos en formato "D<id>-<destino>".
    """
    ...
    zone_occupancy: dict[str, int] = Field(
        default_factory=dict, description="Nombre de zona -> cantidad de drones"
    )
    ...
    def set_connection_occupancy(self, conn: Connection, count: int) -> None:
        ...
        """
        if count < 0:
            raise ValueError(f"Ocupación no puede ser negativa")
        ...
        key = self._normalize_connection_key(conn.zone_a.name, conn.zone_b.name)
